...
six D1 tables are deleted; migration 0019 drops them and adds a `client_kind` column so one table
answers for every client. Migration 0021 renamed that table `sessions` and its kinds are `web`,
`quotabar`, and `ios`.
...
sessions, Devices, observations, Usage, the daily rollup, and the Account.
...
less code than adapting a framework, and it removed the only encrypted-at-rest store, the only
secondary session store, and 1.6 MiB of Worker bundle.
...
the whole credential for ninety days, revocable by sign-out, Delete Account, or expiry.

## When to revisit

If a second sign-in method is scheduled, evaluate a library again at that point — as an identity
front behind `WebSessionPort`, still writing `sessions` — rather than carrying one now for a method
that is not planned. Session ownership does not move back either way.
