...
| Claude Auto | Planner oauth→cli→web | OAuth→browser。cookie 与 CodexBar 一致（`sessionKey`，值须以 `sk-ant-` 开头），另加可选 `lastActiveOrg` 作组织提示——CodexBar 无此 cookie，只从 `/api/organizations` 响应体选组织；校验多一步：组织列表通过后 usage 还要能解析。**已对齐（限定触发）**：CodexBar 用 PTY 跑 `/status` 取额度并顺带触发续期；Quota 在凭据过期或距过期不足一分钟且 `claudeAiOauth` 里有非空 `refreshToken` 时，或文件里没有可用 grant 而 Keychain 条目存在但本进程被拒时，跑一次 `claude mcp list` 让 Claude Code 自己续期，随后重读凭据。**实现差异**：不跑 PTY，也不用 CLI 取额度——`mcp list` 只为触碰 CLI 的 refresh 路径（实测 2.1.246：`auth status --json` 不续期；`doctor` 只在环境里带着运行中会话的变量时才续期；`mcp list` 在 `HOME`/`PATH`/`TERM=dumb`/`CLAUDE_CONFIG_DIR` 的最小环境下稳定续期，且凭据未过期时不动文件）；定时刷新每小时至多一次（`cache.sqlite` 一张 provider→attempt 表记时间 + 结果），Recheck 或手动刷新跳过该小时；10 秒、64 KiB 读完即丢、空的私有 cwd。副作用一条：`mcp list` 会健康检查已批准的 MCP server，空 cwd 把范围限制在 `~/.claude.json` 的 user 级 server，超时兜底。**有意差异**：Keychain 被拒仍跑 `mcp list`（Claude Code 能读这个进程读不到的 grant），不会因此去读 cookie；续期后仍过期报 `auth_required` 并指向打开 Claude Code；CLI 把凭据清空且 Keychain 里也没有条目才报「Claude Code is signed out」，指向终端；Keychain 被拒且文件仍不可用则报 `access_denied`，指向打开 Claude Code |
