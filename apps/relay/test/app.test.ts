...
describe("managed Relay on real Workers and D1", () => {
  it("refuses to rebuild an hour this device's deletion watermark covers", async () => {
    ...
    expect(
      await db
        .prepare("SELECT bucket_start_utc FROM usage_hourly WHERE device_id = 'device_watermark'")
  ...
  it("keeps the daily rollup equal to the hours behind it", async () => {
    ...
    expect(
      await db
        .prepare("SELECT COUNT(*) AS count FROM usage_daily WHERE device_id = 'device_rollup'")
        .first("count"),
    ).toBe(2);
  });
  ...
  it("refuses an upload from a generation the device has moved past", async () => {
    ...
    expect(
      await db
        .prepare("SELECT COUNT(*) AS count FROM usage_hourly WHERE device_id = 'device_stale'")
        .first("count"),
    ).toBe(0);
  });
  ...
  it("answers an unchanged Account read from its validator instead of folding Usage again", async () => {
    ...
    await db
      .prepare("UPDATE accounts SET display_label = 'octocat', updated_at = ?2 WHERE id = ?1")
  ...
  it("turns the activity ETag over when the Usage fold version changes", async () => {
    ...
    expect(first.status).toBe(200);
    const etag = first.headers.get("ETag");
    const bumped = await appFor("account_activity_fold", now, db, {
      usageFoldVersion: 2,
  ...
  it("answers an account whose stored reading this build cannot read", async () => {
    ...
    const stored = await new D1AccountState(db).listLatestSnapshots("account_unreadable");
  ...
  it("expires one rate-limit window without resetting that subject's live one", async () => {
    ...
    const rows = await db
      .prepare("SELECT window_started_at FROM rate_limit_counters ORDER BY window_started_at")
  ...
  it("completes browser PKCE through a Web principal and issues one session", async () => {
    ...
    expect(
      await db
        .prepare("SELECT COUNT(*) AS count FROM sessions WHERE device_id = ?1")
        .bind(tokens.device_id)
        .first("count"),
    ).toBe(1);
    ...
    const lastSeen = () =>
      db
        .prepare("SELECT last_seen_at FROM devices WHERE id = ?1")
        .bind(tokens.device_id)
        .first("last_seen_at");
    const before = "2026-08-09T00:00:00.000Z";
    await db
      .prepare("UPDATE devices SET last_seen_at = ?2 WHERE id = ?1")
    ...
    await db
      .prepare("UPDATE devices SET signed_out_at = NULL WHERE id = ?1")
      .bind(tokens.device_id)
      .run();
    ...
    await db
      .prepare("UPDATE devices SET signed_out_at = NULL, deleted_at = NULL WHERE id = ?1")
      .bind(tokens.device_id)
      .run();
    await db
      .prepare("UPDATE sessions SET revoked_at = NULL WHERE device_id = ?1")
      .bind(tokens.device_id)
      .run();
    ...
    await db.prepare("DELETE FROM accounts WHERE id = ?1").bind(tokens.account_id).run();
    expect(
    ...
    expect(
      await db
        .prepare("SELECT COUNT(*) AS count FROM accounts WHERE id = ?1")
        .bind(tokens.account_id)
        .first("count"),
    ).toBe(0);
  });
  ...
  it("the replaced token can end the family while its successor is unspent", async () => {
    ...
    expect(
      await db
        .prepare("SELECT signed_out_at FROM devices WHERE id = ?1")
...
function storedHour(bucket: string): RelayStatement[] {
  return [
    db
      .prepare(
...
function storedDay(date: string): RelayStatement {
