...
test("the /my document load does not start account/summary; the client asks once with tz", () => {
  ...
  const server = readFileSync(join(root, "../src/routes/my/+page.server.ts"), "utf8");
  const page = readFileSync(join(root, "../src/routes/my/+page.svelte"), "utf8");
  ...
  assert.match(page, /from "\$lib\/account-client"/);
