...
a canonical-JSON SHA-256 of `{fold, account, tz, local_date, devices, device_generation,
usage_revision, pricing_revision, model_catalog_revision}`. `fold` is `USAGE_FOLD_VERSION` (1),
bumped when `buildAccountUsage` changes what it folds. Retention sweeps folds whose `created_at` is
older than two days; a fold never outlives the local date in its key by more than retention.
