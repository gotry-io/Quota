...
final class QuotaUITests: XCTestCase {
  ...
  func testContentFixtureShowsOverview() throws {
    ...
    // wherever a swipe happened to stop would sample a different screen each run.
  ...
  private func performAudit(
    ...
    try app.performAccessibilityAudit(for: types) { issue in
      ...
      // The frames say which overlay or edge an issue sits against, which is what decides
      // whether it is a system-owned report or an app-owned layout bug.
      ...
      XCTFail(
        "\(description) — \(element) frame=\(issue.element?.frame ?? .zero) "
          + "bars=\(app.navigationBars.allElementsBoundByIndex.map { $0.frame }) "
          + "tab=\(app.tabBars.firstMatch.frame) — \(issue.detailedDescription)"
      )
      return true
    }
  }
