...
| Other Usage Data | Yes. Normalized remaining-quota observations (provider, plan/label, windows, reset, observed-at) — the ones this iPhone reads from providers it is signed in to, and the ones Macs running QuotaBar report — and sparse hourly Usage from Macs (token totals, derived API-equivalent cost, completeness). No prompts, completions, paths, credentials, or conversation ids. The Account settings document is also stored: opaque 12-hex subscription selectors, remaining-percent thresholds, reset/pace switches, an optional monthly budget amount, and whether quota history follows the Account — never a provider name, fingerprint, or credential in that document. While that history switch is on, downsampled remaining-quota buckets (provider, fingerprint, window, remaining percent; no credential, no label) are kept as long as the chart for that window shows — two days for a five-hour window, up to thirty days for weekly and monthly ones. Default is on the device; nothing is uploaded until the switch is on; turning it off deletes what Relay holds. | Yes | No | App Functionality (Overview, Today Usage, widgets, multi-device sync, alert policy, budget, optional shared quota history) | Quota observations: 7 days after the instant they describe (readers stop treating them as current after 1 day). Usage hours: 400 days. Daily rollup: 800 days (`all` answers at most 730 days). Usage folds: 2 days. Account settings: until Delete Account. Quota history buckets: as long as the chart for that window shows (two days for a five-hour window, up to thirty days for weekly and monthly ones), or until the switch is turned off. | QuotaRelay. Written by this iPhone (quota only, and history buckets while the switch is on) and by QuotaBar (quota, Usage, and history buckets), read by both and by the website. Settings are written by any signed-in client. | Delete Account (all rows, including settings and history). Delete Device (that Device’s rows, watermark, history). Turning the history switch off deletes uploaded buckets. iOS last-good cache and widget snapshot clear on Log Out. |
