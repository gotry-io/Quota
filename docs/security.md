...
- Persist GitHub subjects, installation identities, token and grant secrets, session-store keys, and
  ...
  in their `Set-Cookie`.
- Retained business data is limited to Account and Device lifecycle metadata, one optional public
  profile row per Account, normalized quota observations, sparse hourly Usage rows, the daily
  rollup, and bounded rate limits. Nothing is kept
