...
expired session, the four tabs, and each widget family in placeholder, content, and no-data states.
...
and `usage-activity` fixture screenshots to `dist/ios-ui-screenshots/`.
...
| `content` | Signed-in Overview with synthetic Codex / Claude / Grok windows and Today values; Usage has four periods with increasing totals, one provider group of more than five models, and an in-memory Activity heatmap of the last 365 UTC days |
