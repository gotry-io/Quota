...
async function seedRetainedSessions(): Promise<void> {
  ...
  const accountSession = db.prepare(
...
describe("0021 one session per client", () => {
  it("carries every retained login into one table and drops what no longer exists", async () => {
    ...
    const revoked = await db
      .prepare("SELECT revoked_at FROM sessions WHERE revoked_at IS NOT NULL")
    ...
    const grants = await db
      .prepare("SELECT id, client_id FROM login_grants ORDER BY id")
      .all<Record<string, unknown>>();
    expect(grants.results).toEqual([
      { id: "grant-browser", client_id: "quotabar" },
      { id: "grant-ios", client_id: "quota-ios" },
    ]);
    const grantColumns = await db.prepare("PRAGMA table_info(login_grants)").all<{
      name: string;
    }>();
    ...
    const platforms = await db
      .prepare("SELECT id, platform FROM devices ORDER BY id")
      .all<Record<string, unknown>>();
