...
async function expectFile(path: string): Promise<void> {
  // Vitest runs provider suites concurrently, and macOS can delay a newly spawned shell under load.
  // The assertion below tests process termination, not startup latency, so allow the marker setup a
  // wider scheduling window while keeping the RPC request timeout itself at 30 ms.
