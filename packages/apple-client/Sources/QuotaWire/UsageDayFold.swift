...
/// its own, so a folded period has no breakdown and says so by carrying no agents, and it carries
/// no cache saving either.
public enum UsageDayFold: Sendable {
  ...
  public static func period(_ days: [UsageActivityDay]) -> UsagePeriod {
    let cost = cost(days.map(\.cost))
    return UsagePeriod(
      totals: totals(days.map(\.totals)),
      cost: cost,
      cacheSaved: cacheSaved(cost),
  ...
  public static func cacheSaved(_ cost: UsageCostOutcome) -> UsageCacheSaved {
    let rows = cost.calculatedRows + cost.reportedRows + cost.unpricedRows
    return UsageCacheSaved(
      amountMicrousd: rows == 0 ? "0" : nil,
