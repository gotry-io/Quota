...
- `GET /api/v6/public/<handle>/usage` is the one route answered `Cache-Control: public, max-age=300`,
  ...
  Relay entry has decided what to trust (only `RELAY_CLIENT_ADDRESS_HEADER` from
  `RELAY_TRUSTED_PROXIES`, otherwise the socket peer; production reads `CF-Connecting-IP` from
