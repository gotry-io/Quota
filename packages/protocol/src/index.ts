...
import {
  acceptsReleasedUnknownUsageModel,
  isReleasedUnknownUsageModel,
  RELEASED_UNKNOWN_USAGE_MODEL,
...
export const MAXIMUM_USAGE_MODELS_PER_SUBMISSION = 64;
...
const ModelSchema = z.string().min(1).max(128).regex(MODEL_PATTERN);
...
export const UsageHourlyFactSchema = UsageHourlyFactWireSchema.superRefine((fact, context) => {
  if (isReleasedUnknownUsageModel(fact.model)) {
    context.addIssue({
      code: "custom",
      path: ["model"],
      message: "The released unknown model sentinel is not valid for current facts.",
    });
  }
});
...
export const UsageSubmissionSchema = z
  ...
  .superRefine((submission, context) => {
    ...
    for (const [index, row] of submission.rows.entries()) {
      if (!acceptsReleasedUnknownUsageModel(submission.parser_revision, row.model)) {
        context.addIssue({
          code: "custom",
          path: ["rows", index, "model"],
          message: "The released unknown model sentinel requires its shipped parser revision.",
        });
      }
    ...
    if (models.size > MAXIMUM_USAGE_MODELS_PER_SUBMISSION) {
      context.addIssue({
        code: "custom",
        path: ["rows"],
        message: `A submission may contain at most ${MAXIMUM_USAGE_MODELS_PER_SUBMISSION} models.`,
      });
    }
  });
...
function validateCostOutcome(
  ...
  if (outcome.unpriced.reduce((sum, item) => sum + item.rows, 0) !== outcome.unpriced_rows) {
    context.addIssue({
      code: "custom",
      path: ["unpriced"],
