...
2. Connect with GitHub and Log Out are the only account actions this device performs. Delete Account
   starts on the website after a fresh GitHub sign-in. Signing in is an invitation inside the app,
   never a wall in front of it: without an account the tabs still open on whatever this phone read
   for itself.
